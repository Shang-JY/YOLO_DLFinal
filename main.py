...
if RESUME:
    ...
    history.load('./history/'+ NAME +"_history.pkl")
    history.summary()

criterion = YoloLoss(BATCH_SIZE,BBOX_NUM,CLASS_NUM,lambda_coord= 0.5, lambda_noobj = 0.5)
...
def train(epoch):
    ...
    mAP = voc_eval(preds,origin,VOC_CLASSES=VOC_CLASSES)
    end_time=time.time()
